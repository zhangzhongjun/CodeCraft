...
def easy_pre(flavor_list,test_time):
    ...
    mean_add = (flavor_list[-1] - flavor_list[int(len(flavor_list) * 0.9)]) / float(len(set(flavor_list[int(len(flavor_list) * 0.9):])) -1)


    predict3 = math.ceil(mean_add * test_time)

    mean_add = ( flavor_list[-1] -  flavor_list[int(len(flavor_list) * 0.6)]) / float(len(set(flavor_list[int(len(flavor_list) * 0.6):])) -1)
