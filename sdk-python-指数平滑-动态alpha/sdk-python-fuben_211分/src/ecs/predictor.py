...
def predict_vm(ecsDataPath, inputFilePath,resultFilePath,dict_k,threshold):
    ...
    for index, f_n in enumerate(flavor_name):
        ...
        '''
        # pre = math.ceil((week_day_pre(start_weekday, labelMat, start_test_weekday, test_time))
        ...
        if pre !=0 :#pre != 0
            ...
            all_hat_name.extend([f_n] * pre)
        pre_dic[f_n] = pre  # {"flavor" :pre,....}
    print(list(sorted(pre_dic.items(),key = lambda x:x[0])))

    result,delet = get_memory_allocation(zero_one,guige[zero_one], guige[1 - zero_one], all_hat, all_hat_limit, all_hat_name)
